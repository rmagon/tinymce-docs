...
Alternatively, [download a language pack](https://www.tiny.cloud/get-tiny/language-packages/) to enable a language other than English (US) and [specify its location]({{ site.baseurl }}/configure/localization/#language_url) with the `language_url` configuration option.
